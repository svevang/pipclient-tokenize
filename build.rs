...
fn main() {
    ...
    let _res = cpython::pip_helpers::install_python_package(python_code_path, Some(requirements_path));
    ...
    locals.set_item(py, "nltk", py.import("nltk").unwrap()).unwrap();

    let installed: bool = py.eval(
        "nltk.download('punkt')",
        None,
        Some(&locals),
